...
static void paint_voronoi( Mat& img, Subdiv2D& subdiv )
{
    ...
    for( size_t i = 0; i < facets.size(); i++ )
    {
        ...
        color[1] = rand() & 255;
        color[2] = rand() & 255;
        fillConvexPoly(img, ifacet, color, 8, 0);//填充多边形
...
int main( int, char** )
{
    ...
    for( int i = 0; i < 20; i++ )
    {
        ...
        locate_point( img, subdiv, fp, active_facet_color );//找到最近的一条＊＊边，然后找到包围三角形并画出三角形和点
        imshow( win, img );
        ...
        img = Scalar::all(0);//归0重画
        draw_subdiv( img, subdiv, delaunay_color );//画所有的三角形包括新增加的
        imshow( win, img );
